                ...
                {projects.map(project => (
                    ...
                    >
                        ...
                        <p className="truncate text-sm md:text-base ">
                            by{' '}
                            <a target="_blank" className="text-green-500 hover:underline" href={project['GitHub Profile URL']}>
                                {project.owner_name}
                            </a>{' '}
                        </p>

                        <p className="mb-2 mt-2 line-clamp-4 text-xs text-gray-400 md:text-sm">{project.project_description}</p>

                        <p className="mb-6 flex flex-wrap gap-x-1 gap-y-1.5">
                            {project.technology_used.split(',').map((tech, index) => (
                                <span
                                    key={index}
                                    className="rounded-full border border-primary-500/50 bg-primary-500/20 px-2 py-0.5 text-xs"
                                >
                                    {tech}
                                </span>
                            ))}
